...
class Repayloanbreakdown extends StatefulWidget {
  FinanceBloc bloc;
  List<LoanScheduleForRefundResponse> loanRequests;
  bool isRequestBreakdown;
  Widget? bottomSheet;
  LoanController controller;
   Repayloanbreakdown({super.key, required this.isRequestBreakdown,
     required this.controller,
     this.bottomSheet,
     required this.bloc,required this.loanRequests});

  @override
  State<Repayloanbreakdown> createState() => _RepayloanbreakdownState();
}

class _RepayloanbreakdownState extends State<Repayloanbreakdown> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _calculateLoanAmounts();
      _determineCurrentMonthPay();
      _filterLoans();
    });
  }

   getCurrentPay() {
   loanPosition=  (totalLoanAmountPaid / totalLoanAmount) * 3.15;
   }

     void _calculateLoanAmounts() {
       double totalAmount = 0.0;
       double paidAmount = 0.0;

       for (var element in widget.loanRequests) {
         double amount = element.totalAmount is String
             ? double.parse(element.totalAmount.replaceAll(",", ""))
             : element.totalAmount;

         totalAmount += amount;

         if (element.paymentStatus.toLowerCase() == "paid") {
           paidAmount += amount;
         }
       }
      getCurrentPay();
       ...
       setState(() {
         ...
         totalLoanAmountPaid = paidAmount;
         loanPosition = loanPosition; // Not needed unless modified
       });

       print("Total Loan Amount Paid: $totalLoanAmountPaid");
       print("Total Loan Amount: $totalLoanAmount");
       print("Value with respect to 3.15: ${getCurrentPay()}");
       print("Value with respect to 3.15223: $loanPosition");
     }

     void _determineCurrentMonthPay() {
       for (var element in widget.loanRequests.reversed) {
         if (element.paymentStatus.toLowerCase() == "outstanding") {
           setState(() => currentMonthPay = element);
           return;
         }
       }
       setState(() => currentMonthPay = widget.loanRequests.last);
     }

     void _filterLoans() {
       setState(() {
         paidLoans = widget.loanRequests
             .where((element) => element.paymentStatus.toLowerCase() == "paid")
             .toList();

         unpaidLoans = widget.loanRequests
             .where((element) => element.paymentStatus.toLowerCase() == "outstanding")
             .toList();
       });
     }
                           ...
                           Center(child:   Text(UcpStrings.fetchingData,
                             ...
                             => Padding(
                               ...
                               child: Column(
                                 children: [
                                   Gap(24.h),
                                   RefundWidget(data: element,index: index+1,total:totalLoanAmount,),
                                 ],
                               ),
                             )).toList(),
                           ),
